...
public class GenericsDao {
		...
		try {
			...
			while (rs.next()) {
				...
				for (int i = 0; i < count; i++) {
					...
					Method method = clazz.getMethod(getSetter(fieldName),
							field.getType());
	...
	public static void main(String[] args) {
		// List<Person> list=BaseDao2.queryList("select * from person ", null,
		// new Person());
		// System.out.println(list.size());
		Person person = queryPo("select * from person where id=?", 4,
				new Person());
