...
public class SimpleMain {
    ...
    private void readWriteData() throws Exception {
        ...
        System.out
                .println("Should have found 1 account matching our query accounts.size()="
                        + accounts.size());
        ...
        System.out.println("Should have found 1 account in for loop accountC="
                + accountC);

        // construct a query using the QueryBuilder
        QueryBuilder<Account, Integer> statementBuilder = accountDao
                .queryBuilder();
        ...
        System.out
                .println("Should not have found any accounts matching our query,accounts.size()="
                        + accounts.size());

        // should find our account: name LIKE 'Jim%' should match our account
        statementBuilder.where().like(Account.NAME_FIELD_NAME,
                name.substring(0, 3) + "%");
        accounts = accountDao.query(statementBuilder.prepare());
        System.out
                .println("Should have found 1 account matching our query,accounts.size()="
        ...
        System.out.println("account was deleted, shouldn't find any,null="
    ...
    private void readWriteBunch() throws Exception {
        ...
        System.out
                .println("Should have found same number of accounts in map, accounts.size()="
                        + accounts.size() + ",all.size()=" + all.size());
        for (Account account : all) {
            System.out.println("Should have found account in map,true="
        ...
        System.out
                .println("Should have found the right number of accounts in for loop , accounts.size()="
                        + accounts.size() + ",accountC=" + accountC);
    }
    ...
    private void useSelectArgFeature() throws Exception {
        ...
        String name2 = "bar";
        String name3 = "baz";
        System.out.println("name1 return num="
                + accountDao.create(new Account(name1)));
        System.out.println("name2 return num="
                + accountDao.create(new Account(name2)));
        System.out.println("name3 return num="
                + accountDao.create(new Account(name3)));
        ...
        System.out
                .println("Should have found 1 account matching our query,results.size()="
                        + results.size());
        System.out.println("name1=" + name1 + ",results.get(0).getName()="
        ...
        System.out
                .println("Should have found 1 account matching our query,results.size()="
                        + results.size());
        System.out.println("name2=" + name2 + ",results.get(0).getName()="
                + results.get(0).getName());
        ...
        System.out
                .println("Should have found 1 account matching our query,results.size()="
                        + results.size());
        System.out.println("name3=" + name3 + ",results.get(0).getName()="
                + results.get(0).getName());
    }
    ...
    private void useTransactions(ConnectionSource connectionSource)
        ...
        System.out.println("accountDao.create return num="
                + accountDao.create(new Account(name)));
        ...
        try {
            ...
            transactionManager.callInTransaction(new Callable<Void>() {
                ...
                public Void call() throws Exception {
                    // we do the delete
                    System.out.println("accountDao.delete return num="
                            + accountDao.delete(account));
                    System.out.println("accountDao.queryForId return null="
                            + accountDao.queryForId(account.getId()));
                    // but then (as an example) we throw an exception which
     ...
     * expected object.
     */
    private void verifyDb(int id, Account expected) throws SQLException,
            Exception {
        // make sure we can read it back
        Account account2 = accountDao.queryForId(id);
        if (account2 == null) {
            throw new Exception("Should have found id '" + id
                    + "' in the database");
        }
    ...
    private static void verifyAccount(Account expected, Account account2) {
        System.out
                .println("assertEquals expected name does not equal account name,expected="
                        + expected + ",account2=" + account2);
        System.out
                .println("assertEquals expected password does not equal account name,expected.getPassword()="
